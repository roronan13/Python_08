...
def show_packages() -> None:
    ...
    if importlib.util.find_spec("pandas"):
        try:
            import pandas
            print(f"[OK] pandas (\
{importlib.metadata.version('pandas')}) - Data manipulation ready.")
...
{importlib.metadata.version('matplotlib')}) - Visualization ready.")
        except ImportError as e:
...
- Numerical computation ready.")
    ...
    if all_installed is False:
        if installed_with_poetry():
            print("   (You must run <poetry install> with every module \
present in pyproject.toml)")
        else:
            print("   (You must run <pip install -r requirements.txt> \
with every module present in requirements.txt)")
...
thanks for creating one before installing packages.")
    ...
    if importlib.util.find_spec("numpy") is None or \
       importlib.util.find_spec("matplotlib") is None or \
    ...
    data_frame = pandas.DataFrame({"timestamp": timestamps,
    ...
    matplotlib.pyplot.plot(data_frame["timestamp"], data_frame["energy"],
                           color="green", label="Matrix energy")
    matplotlib.pyplot.title("Matrix Energy Analysis")
